...
function tipoDeLetras(frase) {
  ...
  if (condicion === true) {
    document.writeln(`<p class="text-center text-danger fs-1">No ingresó una cadena de texto válida</p>"`);
  }else{
    document.writeln(`<h5 class="display-3 text-center">Resultado de la función</h5>"`);
    document.writeln(`<p class="text-center text-success fs-1">${resultado}</p>"`);
  }
